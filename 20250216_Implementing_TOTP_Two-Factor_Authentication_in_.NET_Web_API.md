...
public async Task<IActionResult> UpdateAuthenticatorForUser(AuthenticatorUpdateRequest request)
{
    ...
    currentUser.AuthenticatorKey = request.SecretKey;
...
public async Task<IActionResult> LoginWithOtp(LoginWithOtpRequest request)
{
    ...
    var secretKey = EncryptionHelper.Decrypt(user.AuthenticatorKey, user.Email);
    var isValidOtp = IsOtpValid(secretKey, request.OTP);
    if (!isValidOtp)
        return BadRequest("Invalid OTP.");
